...
> Estos eventos deben llamar a un método de un *contexto* para que este contexto llame a un *servicio* (si lo requiere).
...
|   `AppRouter.jsx`    | **Definir y organizar las rutas principales**. Aplica layouts (público, privado, etc) y define que rutas están protegidas mediante `ProtectedRoute`.                                                          | hacer manejo de sesión ni peticiones al backend.                                                             |
| `ProtectedRoute.jsx` | **Proteger rutas privadas**. Verifica si el usuario está autenticado (por ejemplo: leyendo del contexto). Y redirige o renderiza segun la sesión.                                                             | Realizar el `fetch()` al backend ni modificar el estado global.                                              |
